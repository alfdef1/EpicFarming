...
public enum CropType {
	...
	WHEAT("Wheat", Material.WHEAT, Material.WHEAT, Material.SEEDS),

	CARROT("Carrot", Material.CARROT, Material.CARROT, Material.CARROT),

	POTATO("Potato", Material.POTATO, Material.POTATO, Material.POTATO),
	...
	NETHER_WARTS("Nether Wart", Material.QUARTZ_BLOCK, Material.NETHER_WARTS, Material.NETHER_WARTS);
	...
	public static boolean isCropSeed(Material material) {
		for (CropType type : values())
			if (type.getSeedMaterial() == material)
				return true;
		return false;
	}
